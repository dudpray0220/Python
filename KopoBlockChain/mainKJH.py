...
def writeBlockchain(blockchain):
    ...
    # from 100 to 112, it's reading some existed file sequance.
    try:
        ...
        if blockchainList != []:
            try:
                ...
                with open(g_bcFileName, "w", newline='') as file:
                    # for prevent errors, it backups ur existing file and read.
...
def readBlockchain(blockchainFilePath, mode = 'internal'):
    ...
    try:
        # for after first exec. it will add 'block' on importedBlockchain, which contains 6 'line's. ('line' is List.)
        ...
        # for first exec. it will make csv. file
        if mode == 'internal' :
...
def mineNewBlock(difficulty=g_difficulty, blockchainPath=g_bcFileName):
    # load diffculty from upper named 'g_difficulty', and name's default value is filename.
...
def newtx(txToMining):
    # change string which came from external
    ...
    def do_GET(self):
        ...
        if None != re.search('/block/*', self.path):
            if None != re.search('/block/getBlockData', self.path):
               # 20190605 / (kyuin Park, jiweon Lim, sunghoon Oh, sol Han )
               # ExampleURL : http://localhost:8666/block/getBlockData?range=False&from=1&end=10&recent=False&search=True=3
               # 구간검색, 최근추가기록 검색, 특정인덱스 블록 검색 (단일기능)
               # 구간검색 : 블록 요청 from에 음수값, 0값, 구간 초과요쳥 시 예외 처리
               # 최근기록검색 : blockchain.csv 기준 가장 마지막 블록 검색
               # 특정검색 : 특정 블록 인덱스가 리스트에 없을 시 예외처리

                queryString = urlparse(self.path).query.split('&')
                rangeSet = queryString[0].split('=')[1]
                startPoint = int(queryString[1].split('=')[1]) - 1
                endPoint = int(queryString[2].split('=')[1])
                recentSet = queryString[3].split('=')[1]
                searchSet = queryString[4].split('=')[1]
                options = [rangeSet, recentSet, searchSet]
                block = readBlockchain(g_bcFileName, mode='external')
                maxIndex = int(block[-1].index) + 1

                if options.count("True") >= 2 or options.count("True") == 0 :
                    block = None
                    data.append("check your options. only 1 option available.")
                    data.append("options : range, recent, search.")

                elif options.count("True") == 1 :
                    getBlockTriger = list(filter(lambda x : options[x] == "True", range(len(options))))
                    getBlockTriger = getBlockTriger[0]

                    if getBlockTriger == 0 : # rangeSet, 범위검색
                        if startPoint + 1 > endPoint : # 시작점이 더 클 경우 출력하지 않음.
                            block = None
                            data.append("Wrong input values. check your 'from' and 'end' values.")

                        elif startPoint < 0 : # 시작점이 음수일 경우 0부터 시작
                            startPoint = 0
                            data.append("unexpacted requests, startPoint has set 0.")

                        elif endPoint > maxIndex : # 초과 요청 시
                            endPoint = maxIndex + 1
                            data.append("'end' has out of last index of block. we able to send till " + str(maxIndex) + " blocks.")

                    if getBlockTriger == 1 : # recentSet, 가장 최근블록 검색
                        startPoint = maxIndex - 1
                        endPoint = maxIndex
                        data.append("send recently added blocks.")

                    if getBlockTriger == 2 : # searchSet, 특정 인덱스 블록 검색
                        idxNumber = int(queryString[4].split('=')[2])
                        startPoint = idxNumber - 1
                        endPoint = idxNumber
                        data.append("you just searched block " + str(idxNumber) + ".")

                        if idxNumber > maxIndex or idxNumber <= 0 : # 특정 인덱스블록이 없는 상태일 때.
                            block = None
                            data.append("'search' has out of last index of block. "
                                        "we able to send till " + str(maxIndex) + " blocks.")
                ...
                try:
                    ...
                    self.send_header('Content-type', 'application/json')
                    self.end_headers()
                    data.append("Errors occured : index out of condition, we managed your request as we can.")
                    data.append("options(ex) : range=False&from=1&end=10 // &recent=False // &search=True=3")
                finally:
