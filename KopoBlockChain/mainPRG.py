    ...
    def do_GET(self):
        ...
        if None != re.search('/block/*', self.path):
            if None != re.search('/block/getBlockData*', self.path):
                ...
                startPoint = queryString[0].split('=')[1]
                endPoint = queryString[1].split('=')[1]
                if(startPoint.isdigit() is False):#from=에서 숫자가 아닐때
                    data.append("from = is not Number")
                elif(endPoint.isdigit() is False):#end=에서 숫자가 아닐때
                    print("마지막 블록에 숫자를 기입하세요")
                    data.append("end = is not Number")
                else:
                    startPoint = int(queryString[0].split('=')[1])
                ...
                try:
                    ...
                    if block == None:#블록이 없으면
                        data.append("no data exists")
                    elif startPoint <=0:#startPoint가 0이하일때
                        data.append("no data here")
                    elif startPoint > endPoint:#startPoint가 endPoint보다 큰 수일때
                        data.append("start Number is more than end Number")
                    elif len(block) - 1 != endPoint:#블록의 길이가 endPoint와 같지 않을때
                        data.append("out of range")
                        for i in range(startPoint, endPoint):#길이가 같지 않다고 하고 생성된 블록을 보여줌
